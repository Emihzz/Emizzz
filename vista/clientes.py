...
class VentanaDatosClientes(QMainWindow):
    def __init__(self, datos_guardados):
        super().__init__()
        self.setWindowTitle("Datos Guardados - CLIENTES")
    ...
    def init_ui(self):
        ...
        self.tabla_datos.setColumnCount(9)
        self.tabla_datos.setHorizontalHeaderLabels([
            "Teléfono", "Nombre", "Apellido Paterno", "Apellido Materno",
            "Email", "Fecha Nacimiento", "Fecha Registro",
            "Puntos Acumulados", "Activo"
        ])
    ...
    def mostrar_datos(self):
        ...
        for row, cliente in enumerate(self.datos_guardados):
            for col, campo in enumerate(cliente.keys()):
                item = QTableWidgetItem(str(cliente[campo]))
...
class FormularioClientes(QDialog):
    def __init__(self):
        super().__init__()
        ...
        self.campos = ["telefono", "nombre", "apellido_paterno", "apellido_materno", "email",
                      "fecha_nacimiento", "fecha_registro", "puntos_acumulados", "activo"]
        self.entradas = {}
    ...
    def mostrar_ventana_datos(self):
        if not self.datos_guardados:
            ...
            return
        self.ventana_datos = VentanaDatosClientes(self.datos_guardados)
        ...
        if conn:
            try:
                with conn.cursor() as cursor:
                    ...
                    fecha_registro = self.entradas["fecha_registro"].text()
                    puntos_acumulados = self.entradas["puntos_acumulados"].text()
                    activo_texto = self.entradas["activo"].text().strip().lower()

                    # Validar campos obligatorios
                    if not telefono or not nombre or not email:
                        QMessageBox.warning(self, "Error", "Por favor complete todos los campos obligatorios.")
                        return

                    # Validar el campo 'activo'
                    ...
                    sql = """
                        INSERT INTO clientes (
                            telefono, nombre, apellido_paterno, apellido_materno, 
                            email, fecha_nacimiento, fecha_registro, 
                            puntos_acumulados, activo
                        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                    """
                    cursor.execute(sql, (
                        telefono, nombre, apellido_paterno, apellido_materno,
                        email, fecha_nacimiento, fecha_registro,
                        puntos_acumulados, int(activo)
                    ))
                    conn.commit()
                    ...
                    QMessageBox.information(self, "Éxito", "Datos de cliente guardados correctamente.")
                    self.limpiar_datos()
            except Exception as e:
