...
class VentanaDatos(QMainWindow):
    def __init__(self, datos_guardados):
        super().__init__()
        self.setWindowTitle("Datos Guardados - CONFIGURACION")
    ...
    def init_ui(self):
        ...
        self.tabla_datos.setHorizontalHeaderLabels([
            "ID Config", "Nombre", "Valor", "Descripción"
        ])
    ...
    def mostrar_datos(self):
        ...
        for row, config in enumerate(self.datos_guardados):
            for col, campo in enumerate(config.keys()):
                item = QTableWidgetItem(str(config[campo]))
...
class FormularioConfiguracion(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Formulario - CONFIGURACION")
        self.resize(1000, 600)
        self.setMinimumSize(800, 400)
        self.setWindowFlags(self.windowFlags() | Qt.WindowType.WindowMinMaxButtonsHint)
        self.campos = ["id_config", "nombre", "valor", "descripcion"]
        self.entradas = {}
        ...
        if conn:
            try:
                with conn.cursor() as cursor:
                    ...
                    id_config = self.entradas["id_config"].text()
                    nombre = self.entradas["nombre"].text()
                    valor = self.entradas["valor"].text()
                    descripcion = self.entradas["descripcion"].text()

                    # Validar campos obligatorios
                    if not id_config or not nombre or not valor:
                        QMessageBox.warning(self, "Error", "Por favor complete todos los campos obligatorios.")
                        return

                    # Insertar datos en la base de datos
                    sql = """
                        INSERT INTO configuracion (id_config, nombre, valor, descripcion) 
                        VALUES (%s, %s, %s, %s)
                    """
                    cursor.execute(sql, (id_config, nombre, valor, descripcion))
                    conn.commit()
                    ...
                    QMessageBox.information(self, "Éxito", "Datos de configuración guardados correctamente.")
                    self.limpiar_datos()
            except Exception as e:
...
if __name__ == '__main__':
    ...
    ventana = FormularioConfiguracion()
    ventana.show()
    sys.exit(app.exec())