...
def conectar_base_datos():
    try:
        connection = pymysql.connect(
            host='localhost',
            user='root',
            password='Emiz',  # Cambia por tu contraseña real
            database='NETO',
...
class VentanaDatos(QMainWindow):
    def __init__(self, datos_guardados):
        super().__init__()
        self.setWindowTitle("Datos Guardados - PROVEEDORES")
    ...
    def init_ui(self):
        ...
        self.tabla_datos.setColumnCount(7)
        self.tabla_datos.setHorizontalHeaderLabels([
            "ID Proveedor", "Nombre", "RFC", "Dirección",
    ...
    def mostrar_datos(self):
        ...
        for row, proveedor in enumerate(self.datos_guardados):
            for col, campo in enumerate(proveedor.keys()):
                item = QTableWidgetItem(str(proveedor[campo]))
...
class FormularioProveedores(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Formulario - PROVEEDORES")
        self.resize(1000, 600)
        self.setMinimumSize(800, 400)
        self.setWindowFlags(self.windowFlags() | Qt.WindowType.WindowMinMaxButtonsHint)
        self.campos = ["id_proveedor", "nombre", "rfc", "direccion", "telefono", "email", "activo"]
        self.entradas = {}
        ...
        if conn:
            try:
                with conn.cursor() as cursor:
                    ...
                    id_proveedor = self.entradas["id_proveedor"].text()
                    ...
                    if not id_proveedor or not nombre or not telefono:
                    ...
                    sql = """
                        INSERT INTO proveedores (
                            id_proveedor, nombre, rfc, direccion, telefono, email, activo
                        ) VALUES (%s, %s, %s, %s, %s, %s, %s)
                    """
                    cursor.execute(sql, (
                        id_proveedor, nombre, rfc, direccion, telefono, email, int(activo)
                    ))
                    conn.commit()
                    ...
                    QMessageBox.information(
                        self,
                        "Datos Guardados",
                        "Datos de proveedor guardados correctamente!",
