...
class VentanaDatos(QMainWindow):
    def __init__(self, datos_guardados):
        super().__init__()
        self.setWindowTitle("Datos Guardados - CATEGORIAS")
        self.resize(800, 400)
    ...
    def init_ui(self):
        ...
        self.tabla_datos.setHorizontalHeaderLabels([
            "ID Categoria", "Nombre", "Descripción", "Activa"
        ])
    ...
    def mostrar_datos(self):
        ...
        for row, categoria in enumerate(self.datos_guardados):
            for col, campo in enumerate(categoria.keys()):
                item = QTableWidgetItem(str(categoria[campo]))
...
class FormularioCategorias(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Formulario - CATEGORIAS")
        self.resize(1000, 600)
        self.setMinimumSize(800, 400)
        self.setWindowFlags(self.windowFlags() | Qt.WindowType.WindowMinMaxButtonsHint)
        self.campos = ["id_categoria", "nombre", "descripcion", "activa"]
        self.entradas = {}
        ...
        if conn:
            try:
                with conn.cursor() as cursor:
                    ...
                    id_categoria = self.entradas["id_categoria"].text()
                    ...
                    sql = """
                        ...
                        VALUES (%s, %s, %s, %s)
                    """
                    cursor.execute(sql, (id_categoria, nombre, descripcion, int(activa)))
                    conn.commit()
                    ...
                    QMessageBox.information(self, "Éxito", "Datos de categoría guardados correctamente.")
                    self.limpiar_datos()
            except Exception as e:
